...
describe('Mysql2Adapter', () => {
  ...
  it('createTable', () => {
    ...
    expect(conn.schemaCreation.accept(td)).toBe(
      "CREATE TABLE `users` (`id` bigint NOT NULL AUTO_INCREMENT PRIMARY KEY, `name` varchar(255) DEFAULT 'Untitled', `email` varchar(255), `age` int DEFAULT 0, `bio` text, `active` tinyint(1) DEFAULT 1, `birthday` date, `last_active_at` datetime, `created_at` datetime NOT NULL, `updated_at` datetime NOT NULL) ENGINE=InnoDB"
    );
  });
});
